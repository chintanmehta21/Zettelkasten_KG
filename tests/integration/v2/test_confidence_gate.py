...
def _stub_entitlement_gate(monkeypatch):
    ...
    # factory; run_add_zettel_pipeline CALLS it to build the gemini_client=
    # arg even though summarize_url_bundle is mocked. Stub it so the test
    # never depends on real Gemini keys (or on another test leaking
    # GEMINI_API_KEYS into the env — that was the order-dependent failure).
    monkeypatch.setattr(
...
def _drive_bg_to_finalize(url: str, persist: bool, captured: dict):
