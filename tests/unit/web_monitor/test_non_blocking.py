...
def _kwargs(user_id: str = _SAMPLE_UUID) -> dict:
    return {
        "user_id": user_id,
        "display_name": "Naruto",
...
async def test_notify_pricing_visit_via_create_task_does_not_block_caller(slack_webhook_mock):
    """Mirror of the beacon endpoint pattern: schedule via create_task;
    caller returns immediately even if Slack hangs."""
    ...
    with respx.mock(assert_all_called=False) as router:
        ...
        # Simulate the beacon endpoint's create_task pattern.
        t0 = time.perf_counter()
        task = asyncio.get_running_loop().create_task(notify_pricing_visit(**_kwargs()))
