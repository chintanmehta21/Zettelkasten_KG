...
classes. The chain order is correctness-critical (Brotli must be the
innermost compressor; AuthHeaders/SessionMarker/Auth401 must wrap the
response after the body has been compressed; MemoryGuard must short-
circuit BEFORE any route work happens). A future reorder of
``app.add_middleware`` calls would silently break the response contract;
this test makes any such reorder break loudly instead.
"""
...
def test_middleware_registration_order_pinned():
    ...
    added AFTER Brotli compresses; Set-Cookie outside the compressed body;
    MemoryGuard short-circuits BEFORE PostResponseRelease triggers gc).
    """
    app = create_app()
