...
These tests hit real Substack URLs â€” run with ``pytest --live``.
...
async def test_substack_free_article_live():
    """Free Substack article â†’ extracts full content and metadata."""
...
async def test_substack_paid_article_live():
    """Paid Substack article â†’ returns partial content gracefully (no crash)."""
    url = "https://nlp.elvissaravia.com/p/ai-agents-weekly-gpt-53-codex-spark"
    extractor = NewsletterExtractor()
    result = await extractor.extract(url)

    # Should NOT raise â€” either bypass succeeded or partial content returned
...
async def test_substack_native_domain_free_live():
    """Native *.substack.com free article â†’ full content + Substack metadata."""
