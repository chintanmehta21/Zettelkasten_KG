 ...
 * failures so the profile picker can roll back + surface an error. Before R2 the
 * PUT was fire-and-forget with `.catch(()=>{})`, so a 401/404/500 was invisible
 * and the picker toasted success while nothing persisted.
 */
...
function loadHeader(fetchImpl) {
  ...
  // header.js captures `var zkFetch = window.zkFetch || window.fetch` at eval
  // time, so the fetch stub MUST be installed before we evaluate it.
...
describe('R2 ZKHeader.setAvatarById — awaited PUT, propagated failures', () => {
  ...
  it('resolves with {avatar_url} on a 2xx response', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true, json: () => Promise.resolve({ avatar_url: '/artifacts/avatars/avatar_99.svg' }),
    });
    const ZK = loadHeader(fetchMock);
    const out = await ZK.setAvatarById(99, 'tok', PROFILE_ID);
    expect(out.avatar_url).toBe('/artifacts/avatars/avatar_99.svg');
    expect(fetchMock).toHaveBeenCalledOnce();
    const [url, opts] = fetchMock.mock.calls[0];
    expect(url).toBe('/api/me/avatar');
    expect(opts.method).toBe('PUT');
    expect(JSON.parse(opts.body)).toEqual({ avatar_id: 99 });
  });
  ...
  it('writes the localStorage cache under the real profileId, not anon', async () => {
    ...
    expect(localStorage.getItem('zk-avatar-url-' + PROFILE_ID)).toBe('/artifacts/avatars/avatar_09.svg');
