# RAG Chatbot â€” Implementation Plan
...
**Mode:** Documentation-only â€” execute manually at your own pace
**Granularity:** TDD-strict â€” one failing test â†’ one implementation â†’ green, per task
**Phases:** 9 (Phase 0 Preflight + 8 rollout phases from spec Â§10)
...
- **Goal** â€” one sentence, the "definition of done"
- **Dependencies** â€” what the prior phase must have delivered
- **Phase 0 docs discovery** (for each implementation phase) â€” cite exact doc locations, grep commands, and spec sections to read BEFORE writing code
- **Tasks** â€” numbered TDD cycles: each task is `test â†’ implement â†’ green â†’ commit` (one commit per task unless noted)
- **Verification gate** â€” how to prove the phase is done (tests to run, greps to perform, acceptance criteria)
- **Anti-pattern guards** â€” what NOT to do (invented APIs, wrong imports, skipped tests)
- **Rollback** â€” single command to undo the phase

Each task links back to a specific `Â§X.Y` of the spec so you never write code without a source of truth.
...
4. **Commit**: `git add <changed files> && git commit -m "<type>(<scope>): <imperative, â‰¤10 words>"`. One task = one commit unless a task is split across two files and can't be partially committed.
...
- **Do not invent APIs**: if a method isn't in the spec or the documentation discovery output, read the docs before calling it. No `.embed_batch`, no `generate_content_stream` on `GeminiKeyPool`, no `langfuse_context`. Use only the verified interfaces in spec Â§3.7.
- **Do not mock `get_settings()`** â€” always patch it via `@patch` per `CLAUDE.md`. Calling the real one in a test exits the process.
- **Do not bypass the adapter layer** (spec Â§3.7): all Gemini calls go through `website/features/rag_pipeline/adapters/pool_factory.py` + the two helper adapters. Never import `google.generativeai` directly from RAG business logic.
- **Do not add Supabase tables/columns outside the 5 migrations** â€” schema drift from the spec is a review-blocker.
...
- **Adapter layer** = `website/features/rag_pipeline/adapters/` (new in this plan; spec Â§3.7)
- **Phase 3 target** = the internal-only retrieval core, no public surface
- **Phase 5 target** = the first user-visible web endpoint

---

## Phase 0 â€” Preflight, Documentation Discovery, Fixtures
...
- `docs/superpowers/plans/2026-04-12-rag-chatbot-preflight-report.md` â€” a short file recording each verification's outcome
- Confirmed pgvector version on prod Supabase (captured in the report)

### Task 0.1 â€” Verify pgvector â‰¥ 0.8 on prod Supabase
...
**Docs reference:** spec Â§13 open question #1.

**Acceptance:** version string is â‰¥ `0.8.0`. If it's less, stop and upgrade Supabase before proceeding. If upgrade is blocked, open a separate decision doc about the multi-tenant HNSW recall degradation workaround (post-filter CTE). **Do not proceed past this task if the version is unconfirmed.**

**Rollback:** n/a (read-only query).

---

### Task 0.2 â€” Verify Chonkie `BaseEmbeddings` abstract contract
...
Record the exact abstract method names and return types in the preflight report. If Chonkie ships an official `chonkie[gemini]` extra with a `GeminiEmbeddings` class that works with an API-key-based Gemini client, note that â€” `GeminiChonkieEmbeddings` (spec Â§3.7.2) may become a no-op wrapper.

**Docs reference:** spec Â§13 open question #5.

**Acceptance:** a verbatim list of required methods in the report. The adapter in Task 2.5 will implement exactly those.

---

### Task 0.3 â€” Verify TEI supports `BAAI/bge-reranker-v2-m3`
...
**Docs reference:** spec Â§13 open question #2, #3, #4.

**Acceptance:** 3 answers recorded.

---

### Task 0.4 â€” Verify Langfuse v3 `@observe` on async generators
...
**Docs reference:** spec Â§13 open question #6.

**Acceptance:** one of (a) decorator works on async generators â†’ proceed as spec says; (b) it doesn't â†’ document a manual trace pattern (`with langfuse.start_as_current_span(...)`) to use instead in Phase 7.

---

### Task 0.5 â€” Verify RAGAS 0.4.3 Gemini judge via `LangchainLLMWrapper`
...
Expected: a score between 0 and 1 prints. If the wrapper pattern fails, fall back to LiteLLM + Vertex (spec Â§6.3 alt path) â€” document which path was chosen.

**Docs reference:** spec Â§13 open question #9.

**Acceptance:** either path works end-to-end; note which one in the report.

---

### Task 0.6 â€” Generate synthetic corpus fixture
...
```python
# Pseudocode â€” expand manually, not via agent.
...
### Task 0.7 â€” Generate golden Q/A fixture (RAGAS 0.4 schema)

**What to do:** manual task. Write 30 Q/A pairs by hand where each Q can be answered from 1â€“4 of the synthetic Zettels in Task 0.6. Use the **exact** RAGAS 0.4 column names:
...
`retrieved_contexts` stays empty at authoring time â€” the eval harness fills it per run by calling the retrieval pipeline. `ground_truth_support` is the oracle we measure context recall against. Column order doesn't matter.

**Acceptance:** `golden_qa.json` has 30 entries, each with all 5 fields.

---

### Task 0.8 â€” Write the preflight report

**What to do:** consolidate outcomes of 0.1â€“0.7 into `docs/superpowers/plans/2026-04-12-rag-chatbot-preflight-report.md`. Sections: pgvector version, Chonkie abstract contract, TEI smoke-test results, Langfuse async-generator outcome, RAGAS judge path, fixtures row counts. One commit.
...
## Phase 1 â€” SQL migrations 001â€“005

**Goal:** Apply all 5 migrations from spec Â§2 to the production Supabase database without blocking any writes or breaking any existing functionality. Zero Python code changes.

**Dependencies:** Phase 0 complete, pgvector â‰¥ 0.8 confirmed.
...
1. **Read the spec sections verbatim:** Â§2.1 (Migration 001), Â§2.2 (Migration 002), Â§2.3 (Migration 003), Â§2.4 (Migration 004), Â§2.5 (Migration 005).
2. **Read the existing schema** for pattern copies:
   - `supabase/website/kg_public/schema.sql` â€” RLS policy pattern for `kg_nodes`, `kg_links`, `kg_users` (copy this exact shape)
   - `supabase/website/kg_features/001_intelligence.sql:14-16` â€” current IVFFlat index declaration (must be dropped)
   - `supabase/website/kg_features/001_intelligence.sql:161-235` â€” `find_neighbors` recursive CTE (mirror pattern in `rag_hybrid_search` graph_walk CTE)
   - `supabase/website/kg_features/001_intelligence.sql:531-651` â€” current `hybrid_kg_search` RPC (template for the new RPC)
...
### Task 1.1 â€” Create `supabase/website/rag_chatbot/001_hnsw_migration.sql`

**What to do:** copy verbatim from spec Â§2.1 into the new file. One file.

**Docs reference:** spec Â§2.1.
...
### Task 1.2 â€” Create `002_chunks_table.sql`

**What to do:** copy verbatim from spec Â§2.2. One file.

**Docs reference:** spec Â§2.2.
...
### Task 1.3 â€” Create `003_sandboxes.sql`

**What to do:** copy verbatim from spec Â§2.3, including the `rag_sandbox_stats` view and ALL 10 RLS policies. One file.

**Docs reference:** spec Â§2.3.
...
### Task 1.4 â€” Create `004_chat_sessions.sql`

**What to do:** copy verbatim from spec Â§2.4, including the stats-update trigger and RLS pattern. One file.

**Docs reference:** spec Â§2.4.
...
### Task 1.5 â€” Create `005_rag_rpcs.sql`

**What to do:** copy verbatim from spec Â§2.5. All 5 RPCs. Pay close attention to `rag_hybrid_search` â€” it's 180+ lines of plpgsql.

**Docs reference:** spec Â§2.5.
...
### Task 1.6 â€” Apply all 5 migrations to prod Supabase

**What to do:** via the Supabase SQL editor, paste each migration file as a separate execution. In order: 001 â†’ 002 â†’ 003 â†’ 004 â†’ 005. Pause between each and run the verification queries from tasks 1.1â€“1.5 against the prod instance (read-only, no test data).

**Docs reference:** spec Â§10 Phase 1.
...
**Commit:** no code change â€” tag the deploy in the commit message of Task 1.5 with `(applied to prod 2026-MM-DD)` via an amend, OR create an empty commit `chore(rag): apply migrations 001-005 to prod`.
...
**Rollback:** each migration has a commented rollback block at the bottom. Execute them in reverse order (005 â†’ 001). The 001 rollback recreates the IVFFlat index.

---

## Phase 2 â€” Ingestion (chunker + embedder + upsert + telegram wiring)
...
**Deliverables:**
- `website/features/rag_pipeline/types.py`, `errors.py`
- `website/features/rag_pipeline/adapters/pool_factory.py`, `gemini_chonkie.py`
- `website/features/rag_pipeline/ingest/chunker.py`, `embedder.py`, `upsert.py`
- `telegram_bot/config/settings.py` â€” new `rag_chunks_enabled` flag (default False)
- `telegram_bot/pipeline/orchestrator.py` â€” non-blocking chunk-ingest step wired in
- All unit tests for the above

### Phase 0 docs discovery (for Phase 2)

1. Read spec Â§3.1 (shared types), Â§3.2 (chunker/embedder/upsert), Â§3.7.1 (pool_factory), Â§3.7.2 (GeminiChonkieEmbeddings).
...
### Task 2.1 â€” Add `rag_chunks_enabled` setting
...
```
Run it â€” expect `AttributeError: 'Settings' object has no attribute 'rag_chunks_enabled'`.
...
### Task 2.2 â€” Shared types (`website/features/rag_pipeline/types.py`)
...
```
Also `test_scope_filter_default_tag_mode_is_all`, `test_retrieval_candidate_fields`, `test_chat_query_default_quality_is_fast`. Run â€” all fail with `ModuleNotFoundError`.

**Green:** create `website/features/rag_pipeline/types.py` verbatim from spec Â§3.1. Only include the types the tests exercise; leave unused fields out until later tasks need them. (TDD: don't over-implement.)

**Commit:** `feat(rag): shared Pydantic types`

---

### Task 2.3 â€” Error classes (`website/features/rag_pipeline/errors.py`)

**Red:** `tests/unit/rag/test_errors.py::test_error_hierarchy`
```python
def test_empty_scope_error_inherits_from_ragerror():
    from website.features.rag_pipeline.errors import RAGError, EmptyScopeError, LLMUnavailable
    assert issubclass(EmptyScopeError, RAGError)
    assert issubclass(LLMUnavailable, RAGError)
```

**Green:** create `website/features/rag_pipeline/errors.py` with `class RAGError(Exception): pass` and subclasses. Add at least: `EmptyScopeError`, `LLMUnavailable`, `RerankerUnavailable`, `CriticFailure`, `SessionGoneError`.

**Commit:** `feat(rag): error hierarchy`

---

### Task 2.4 â€” Pool factory singleton (`adapters/pool_factory.py`)
...
**Green:** copy verbatim from spec Â§3.7.1. Use `@lru_cache(maxsize=1)`.
...
### Task 2.5 â€” `GeminiChonkieEmbeddings` adapter
...
def test_embed_returns_768d_vectors(monkeypatch):
    ...
    monkeypatch.setattr(
        "website.features.rag_pipeline.adapters.pool_factory.get_gemini_pool",
        lambda: fake_pool
    )
...
**Green:** implement per spec Â§3.7.2. Use the abstract methods from the Phase 0 preflight report â€” if the report found Chonkie uses different method names, use those instead of the spec's `embed` / `embed_query` / `dimension`.

**Anti-pattern guard:** `GeminiChonkieEmbeddings.__init__` must NOT accept a pool parameter â€” it calls `get_gemini_pool()`. Chonkie instantiates it by name.

**Commit:** `feat(rag): GeminiChonkieEmbeddings adapter`

---

### Task 2.6 â€” ZettelChunker (source-type dispatch + atomic enrichment)
...
**Green:** implement per spec Â§3.2 Chunker section (patched version with correct Chonkie parameter names â€” `min_sentences_per_chunk`, no `chunk_overlap` on RecursiveChunker). Use the fallback ladder: late â†’ semantic â†’ recursive â†’ token.

**Anti-pattern guard:** do NOT pass `chunk_overlap` to `RecursiveChunker` (not a valid parameter). Do NOT use `min_sentences=` (wrong name). Use the exact signatures from spec Â§3.2.

**Commit:** `feat(rag): ZettelChunker with source-type dispatch`

---

### Task 2.7 â€” ChunkEmbedder with async-wrapped sync pool
...
**Green:** implement per spec Â§3.2 (patched version using `asyncio.to_thread` + `EmbedContentConfig` + tuple-unpacked response).

**Anti-pattern guard:** do NOT call `await self._pool.embed_batch(...)` â€” this method doesn't exist. Use `asyncio.to_thread(self._pool.embed_content, ...)`.

**Commit:** `feat(rag): ChunkEmbedder async wrapper over sync pool`

---

### Task 2.8 â€” upsert_chunks with content-hash skip
...
**Green:** implement per spec Â§3.2 upsert.py (patched version with content-hash skip path). Use `supabase.rpc("rag_replace_node_chunks", ...)` and `supabase.table("kg_node_chunks").insert(rows).execute()`.

**Anti-pattern guard:** do NOT insert without the delete RPC first â€” that violates the replace-then-insert contract.

**Commit:** `feat(rag): upsert_chunks with hash skip`

---

### Task 2.9 â€” Wire chunk ingest into Telegram pipeline
...
### Task 2.10 â€” Staging validation (manual)

**What to do:** deploy to staging with `rag_chunks_enabled=True` set for that environment only. Capture 10 test URLs via the Telegram bot â€” mix of source types (YouTube x3, Reddit x2, Substack x2, GitHub x2, Twitter x1). After each, query:
...
**Rollback:** set `rag_chunks_enabled=False` in prod env. No data cleanup â€” chunks are a read-optional side channel. If bad data must be cleaned: `DELETE FROM kg_node_chunks WHERE created_at > '<deploy timestamp>'`.

---

## Phase 3 â€” Retrieval core + orchestrator + TEI sidecar (INTERNAL, no surface)
...
1. Re-read spec Â§3.3 (query pipeline), Â§3.4 (retrieval), Â§3.5 (graph score), Â§3.6 (reranker), Â§3.7 (adapter layer), Â§3.8 (TEI sidecar), Â§4.1 (context), Â§4.2 (prompts), Â§4.3 (LLM router + Gemini backend), Â§4.4 (Claude stub), Â§4.5 (Answer Critic), Â§4.6 (session store), Â§4.7 (orchestrator).
2. Re-read the preflight report â€” confirm Langfuse async-generator support path and BGE model choice.
3. Read `website/features/api_key_switching/key_pool.py:175-216` for the existing `generate_content` key-rotation pattern â€” `gemini_stream.py` mirrors this exactly.
4. Read the existing `website/core/supabase_kg/repository.py` RPC call patterns (lines 460-468 for `match_kg_nodes`, etc.) â€” use these for every `rag_*` RPC call.

This phase has ~30 tasks. Batch them into 5 sub-phases by layer:

- **3.A Query layer** (tasks 3.1â€“3.3): rewriter, router, transformer
- **3.B Retrieval layer** (tasks 3.4â€“3.8): cache, resolve, hybrid, dedup, graph scoring
- **3.C Rerank + context** (tasks 3.9â€“3.12): TEI sidecar, TEI client, context assembler
- **3.D Generation + critic** (tasks 3.13â€“3.19): prompts, LLM router, Gemini backend, streaming adapter, Claude stub, critic
- **3.E Orchestrator + memory** (tasks 3.20â€“3.25): session store, full pipeline, error handling, integration test

### 3.A Query layer (3 tasks)

**Task 3.1 â€” QueryRewriter**
...
- **Green:** implement per spec Â§3.3 with `gemini-2.5-flash-lite` via the pool adapter
- **Commit:** `feat(rag): QueryRewriter multi-turn standalone`

**Task 3.2 â€” QueryRouter**
...
- **Green:** implement per spec Â§3.3 â€” flash-lite with `response_mime_type="application/json"`
- **Commit:** `feat(rag): QueryRouter five-class classifier`

**Task 3.3 â€” QueryTransformer (all 5 paths)**
...
- **Green:** implement per spec Â§3.3 (now including the STEP_BACK path from the blueprint-gap fix #1)
- **Commit:** `feat(rag): QueryTransformer with HyDE/MQ/Decomp/StepBack`

### 3.B Retrieval layer (5 tasks)

**Task 3.4 â€” LRUCache with TTL**
...
- **Green:** implement per spec Â§3.4 `cache.py`
- **Commit:** `feat(rag): async LRU cache with TTL`

**Task 3.5 â€” _resolve_nodes (fast path for NULL + RPC for narrowed)**
...
- **Green:** implement the `_resolve_nodes` method of `HybridRetriever` per spec Â§3.4
- **Commit:** `feat(rag): scope resolver with fast path for all-nodes`

**Task 3.6 â€” hybrid retrieval fan-out + consensus boost**
...
- **Green:** implement `HybridRetriever.retrieve` + `_dedup_and_fuse` per spec Â§3.4
- **Commit:** `feat(rag): hybrid retrieve with variant fan-out`

**Task 3.7 â€” Graph depth configuration per query class**
...
**Task 3.8 â€” LocalizedPageRankScorer**
...
- **Green:** implement per spec Â§3.5 with `networkx.pagerank`
- **Commit:** `feat(rag): localized PageRank over retrieved subgraph`

### 3.C Rerank + context (4 tasks)

**Task 3.9 â€” Add TEI sidecar to blue/green compose files**

- **What to do:** copy the YAML from spec Â§3.8 into both `ops/docker-compose.blue.yml` and `ops/docker-compose.green.yml`. Use the model ID chosen in Phase 0 (v2-m3 or large fallback).
- **Test (manual):** `docker compose -f ops/docker-compose.blue.yml up reranker` on staging â†’ wait for `Ready` log â†’ `curl http://localhost:8080/health`.
- **Commit:** `feat(ops): TEI reranker sidecar for blue/green`

**Task 3.10 â€” TEIReranker client**
...
- **Green:** implement per spec Â§3.6 with `httpx.AsyncClient` + `tenacity.retry`
...
**Task 3.11 â€” ContextAssembler with sandwich order + budget**
...
- **Green:** implement per spec Â§4.1
- **Commit:** `feat(rag): context assembler with sandwich ordering`

**Task 3.12 â€” System prompt + user template constants**
...
- **Green:** create `website/features/rag_pipeline/generation/prompts.py` per spec Â§4.2 verbatim
- **Commit:** `feat(rag): system prompt and user template`

### 3.D Generation + critic (7 tasks)

**Task 3.13 â€” GeminiStreamAdapter (key-rotation streaming)**
...
- **Green:** implement per spec Â§3.7.3. Mock `google.genai.Client` entirely in tests â€” don't touch real Gemini.
...
**Task 3.14 â€” GeminiBackend non-streaming path**
...
- **Green:** implement per spec Â§4.3 `generate()` method using the existing pool's `generate_content`
- **Commit:** `feat(rag): Gemini non-streaming backend`

**Task 3.15 â€” GeminiBackend streaming path (wires to adapter)**
...
- **Green:** implement per spec Â§4.5
- **Commit:** `feat(rag): Answer Critic with NLI + citation check`

**Task 3.19 â€” `record_generation_cost` helper**

- **Red:** `test_cost_helper_calls_langfuse_update_current_generation`, `test_unknown_model_uses_zero_cost`
- **Green:** implement per spec Â§6.5 `record_generation_cost` function
- **Commit:** `feat(rag): explicit Gemini cost tracking helper`

### 3.E Orchestrator + memory (6 tasks)

**Task 3.20 â€” ChatSessionStore CRUD**

- **Red:** 6 tests: create/get/list/delete + append_user_message + append_assistant_message. Mock Supabase via fake client.
- **Green:** implement per spec Â§4.6
- **Commit:** `feat(rag): ChatSessionStore CRUD`

**Task 3.21 â€” `load_recent_turns` + `auto_title_session`**

- **Red:** `test_load_recent_turns_returns_oldest_first`, `test_auto_title_uses_first_line_up_to_60_chars`
- **Green:** implement the two methods per spec Â§4.6
- **Commit:** `feat(rag): session history loader + auto-title`

**Task 3.22 â€” Orchestrator `_run_pipeline` happy path (non-streaming)**

- **Red:** `test_answer_happy_path_returns_grounded_answer_with_citations`. Use fully mocked dependencies â€” this test verifies the orchestration wiring, not the retrieval quality.
- **Green:** implement `answer()` and `_run_pipeline(stream=False)` per spec Â§4.7 â€” all 14 steps, but mock everything.
- **Commit:** `feat(rag): orchestrator non-streaming path`

**Task 3.23 â€” Orchestrator empty-scope + LLM-down error paths**

- **Red:** `test_empty_scope_raises_empty_scope_error`, `test_llm_unavailable_propagates`
- **Green:** add the error-handling branches to `_run_pipeline` per spec Â§4.7
- **Commit:** `fix(rag): orchestrator error paths`

**Task 3.24 â€” Orchestrator critic retry loop**
...
- **Green:** implement `answer_stream()` per spec Â§4.7 yielding the event dicts
...
- [ ] TEI sidecar healthy on staging (`docker compose ps reranker` â†’ healthy)
- [ ] p95 latency on a 10-query smoke test < 8s total (retrieval < 500ms, rerank < 400ms, generation < 5s)
- [ ] No direct imports of `google.generativeai` from RAG business logic (grep check: `grep -rE "^from google" website/features/rag_pipeline/` should only match `adapters/` files)
- [ ] No direct imports of `langfuse.decorators` (grep check: `grep -r "langfuse.decorators" website/features/rag_pipeline/` â†’ 0 matches; v3 uses `from langfuse import observe`)

**Rollback:** revert the Phase 3 commits. No production surface to roll back.

---

## Phase 4 â€” Telegram `/ask` (soft launch)
...
### Task 4.1 â€” AskRateLimiter

- **Red:** `test_allow_within_limit`, `test_block_when_over_limit`, `test_window_slides_over_time`
- **Green:** minimal sliding-window class with `allow(chat_id: int) -> bool` per spec Â§5.11
- **Commit:** `feat(telegram): ask rate limiter`

### Task 4.2 â€” `_format_answer_for_telegram` helper
...
- **Green:** implement per spec Â§5.11
- **Commit:** `feat(telegram): ask answer formatter`

### Task 4.3 â€” `ask_command` handler
...
- **Green:** implement per spec Â§5.11. Import `get_orchestrator()` lazily inside the handler to avoid startup cost when `/ask` is never used.
- **Commit:** `feat(telegram): /ask command handler`

### Task 4.4 â€” Register the handler in `main.py`
...
### Task 4.5 â€” Manual prod smoke test

- **What to do:** deploy to prod via existing blue/green workflow. Send 10 `/ask` queries covering: short fact, ambiguous/vague, multi-hop, thematic, one that should fail ("what do my notes say about quantum zeppelins" â€” expect "I can't find anything").
- **Acceptance:** 10/10 answers return within 15s, grounded citations format correctly, the "no results" case handles gracefully.
- **No commit** â€” just capture results in a short `phase4-validation.md` note.
...
## Phase 5 â€” Web `/api/chat/adhoc` + minimal chat UI
...
- `website/features/rag_pipeline/api_models.py` (Pydantic request/response)
...
1. Read spec Â§5.1, Â§5.2, Â§5.3, Â§5.4, Â§5.5, Â§5.6, Â§5.9.
...
### Task 5.1 â€” Pydantic api_models

- **Red:** `test_send_message_request_validates_content_length`, `test_scope_filter_defaults_to_all_none`
- **Green:** copy from spec Â§5.3 into `website/features/rag_pipeline/api_models.py`
- **Commit:** `feat(rag): API Pydantic models`

### Task 5.2 â€” `rag_rate_limiter` buckets

- **Red:** `test_chat_message_bucket_per_user`, `test_retrieve_embedding_bucket_per_user`
- **Green:** extend the existing rate-limiter pattern (copy from `website/api/routes.py:27-96`) with the 5 buckets from spec Â§5.4. New file: `website/api/rag_rate_limiter.py`.
- **Commit:** `feat(rag): API rate limit buckets`

### Task 5.3 â€” `/api/chat/adhoc` endpoint

- **Red:** `test_adhoc_returns_sse_stream`, `test_adhoc_respects_rate_limit`, `test_adhoc_requires_auth`, `test_adhoc_handles_empty_scope`
- **Green:** write `website/api/chat_routes.py` with just the `/api/chat/adhoc` route. Use `StreamingResponse(event_stream(), media_type="text/event-stream")` with the headers from spec Â§5.2. Wire to `RAGOrchestrator.answer_stream()` with `session_id=None, sandbox_id=None`.
...
### Task 5.4 â€” SSE client JavaScript

- **Red:** n/a (no JS test framework in this repo). Write a manual test checklist instead.
- **Green:** copy spec Â§5.9 SSEClient verbatim into `website/features/rag_chatbot/static/js/sse_client.js` as an ES module.
- **Commit:** `feat(rag): SSE client JS module`

### Task 5.5 â€” Minimal chat page
...
- **Design language:** teal accent, amber citation chips â€” NO purple. Consistency with the existing website's Tailwind palette.
- **Commit:** `feat(rag): minimal chat page UI`

### Task 5.6 â€” Register `chat.html` route in `website/app.py`
...
### Task 5.7 â€” Manual staging + prod validation
...
- **Commit:** none â€” capture in `phase5-validation.md`.
...
## Phase 6 â€” Sandboxes + multi-turn + full frontend + KG integration
...
- **6.A Sandbox API** (tasks 6.1â€“6.6)
- **6.B Chat session API extension** (tasks 6.7â€“6.11)
- **6.C Frontend polish** (tasks 6.12â€“6.20)
- **6.D KG integration** (tasks 6.21â€“6.23)

### Phase 0 docs discovery (for Phase 6)

1. Re-read spec Â§1.4 (sandbox concept), Â§5.1 (endpoint catalog), Â§5.5 (frontend layout), Â§5.7 (pages), Â§5.8 (UX flows), Â§5.10 (KG integration), Â§12 edge cases 1â€“46.
2. Read `website/features/knowledge_graph/static/js/` for the existing `kgScene`, `kgSelection` hooks.

### 6.A Sandbox API (6 tasks)

**Task 6.1** â€” `POST /api/rag/sandboxes` + `GET /api/rag/sandboxes` (list)
  - 4 tests (create, list-empty, list-with-entries, unique-name-constraint)
  - Implement per spec Â§5.1 and Â§5.3
  - `feat(rag): sandbox create + list`

**Task 6.2** â€” `GET /api/rag/sandboxes/{id}` (single sandbox via `rag_sandbox_stats` view)
  - 2 tests (exists, 404)
  - `feat(rag): sandbox get by id`

**Task 6.3** â€” `GET /api/rag/sandboxes/{id}/members` (paginated)
  - 3 tests (empty, pagination, filter-by-added_via)
  - `feat(rag): list sandbox members`

**Task 6.4** â€” `POST /api/rag/sandboxes/{id}/members` (add, manual or bulk)
  - 4 tests: explicit node_ids, bulk tag filter, bulk source filter, validation error for empty payload
  - Call `rag_bulk_add_to_sandbox` RPC per spec Â§2.5 RPC #4
  - `feat(rag): sandbox add members with bulk RPC`

**Task 6.5** â€” `DELETE /api/rag/sandboxes/{id}/members/{node_id}` + bulk delete
  - 3 tests
  - `feat(rag): sandbox remove members`

**Task 6.6** â€” `PATCH` and `DELETE /api/rag/sandboxes/{id}` (rename, delete w/ cascade)
...
**Task 6.7** â€” `POST /api/chat/sessions` (create, optionally with `sandbox_id`)
  - 3 tests (ad-hoc, sandbox-tied, 404-for-unknown-sandbox)
  - `feat(rag): chat session create`

**Task 6.8** â€” `GET /api/chat/sessions` + `GET /api/chat/sessions/{id}` + `GET .../messages` (list with pagination)
  - 4 tests
  - `feat(rag): chat session list + get`

**Task 6.9** â€” `POST /api/chat/sessions/{id}/messages` (SSE streaming, scoped to session's sandbox)
  ...
  - **This is the main prod endpoint** â€” test coverage is critical
  - `feat(rag): chat send-message SSE endpoint`

**Task 6.10** â€” `DELETE /api/chat/sessions/{id}` (cascade) + `DELETE .../messages/{mid}` (edit-and-retry fork)
  - 3 tests (cascade, fork from mid-session, 404)
  - `feat(rag): chat session delete + message fork`

**Task 6.11** â€” `PATCH /api/chat/sessions/{id}` (rename, update quality)
  - 2 tests
  - `feat(rag): chat session update`

### 6.C Frontend polish (9 tasks)

**Tasks 6.12â€“6.20** â€” one task per frontend module. These are manual/visual tasks; tests are limited to route-registration and template-renders-without-error checks. Each task:

- **6.12** â€” `session_store.js` client-side cache
- **6.13** â€” `sandbox_manager.js` CRUD + modals
- **6.14** â€” `scope_picker.js` (tag, source, node-id narrowing UI)
- **6.15** â€” `citation_panel.js` (right rail)
- **6.16** â€” `message_renderer.js` (markdown + citation linking)
- **6.17** â€” `chat_controller.js` (top-level coordinator, multi-turn, edit-and-retry)
- **6.18** â€” `sandboxes.html` page + template
- **6.19** â€” `sandbox_detail.html` page
- **6.20** â€” Full `chat.html` integration with left rail + center + right rail

Each commit: `feat(rag): <module name>`

### 6.D KG integration (3 tasks)

**Task 6.21** â€” "Ask about this" right-click in 3D KG page
  - Write `kg_rag_integration.js`, wire `kgScene.onNodeRightClick`
  - Visual test only
  - `feat(kg): ask-about-node context menu`

**Task 6.22** â€” "Add to sandbox" multi-select toolbar
  - Wire to `window.kgSelection` registry
  - Opens sandbox picker modal (reuses `sandbox_manager.js`)
  - `feat(kg): add-to-sandbox toolbar`

**Task 6.23** â€” "View in graph" from citation chip (reverse direction)
...
- [ ] Full end-to-end scenario on staging: create sandbox â†’ add 20 Zettels via 3 different affordances â†’ 5-turn conversation â†’ edit one old message and retry â†’ rename sandbox â†’ cascade-delete sandbox â†’ verify cascade removed chat sessions + messages
- [ ] Visual check: no purple anywhere. Teal/amber only.
- [ ] Spec Â§12 edge cases 38, 44, 45, 46 manually verified
...
## Phase 7 â€” Observability + RAGAS CI eval

**Goal:** Langfuse sidecar running in prod; every RAG turn emits a trace; GitHub Actions runs RAGAS on every PR touching `website/features/rag_pipeline/**`.

**Dependencies:** Phases 1â€“6 complete. Preflight Task 0.4 (Langfuse `@observe` on async generators) and 0.5 (RAGAS Gemini judge path) outcomes known.

### Phase 0 docs discovery (for Phase 7)

1. Re-read spec Â§6.1 (3 eval layers), Â§6.3 (RAGAS in CI), Â§6.4 (Langfuse sidecar), Â§6.5 (instrumentation), Â§6.6 / Â§6.6a (cost tracking).
2. Re-read Phase 0 preflight report rows 4 and 5 (the verified paths).

### Task 7.1 â€” Langfuse + Langfuse-Postgres sidecars in compose files

- Copy spec Â§6.4 YAML into `ops/docker-compose.blue.yml` and `ops/docker-compose.green.yml`
...
### Task 7.2 â€” `tracer.py` + explicit cost helper

- **Red:** `test_trace_stage_decorator_wraps_async`, `test_record_generation_cost_calls_update_current_generation`
- **Green:** implement `website/features/rag_pipeline/observability/tracer.py` per spec Â§6.5 (patched with `from langfuse import observe, get_client`)
- **Commit:** `feat(rag): Langfuse tracer + cost helper`

### Task 7.3 â€” Wire `@trace_stage` decorators + cost recording on orchestrator

- Add `@trace_stage("query_rewrite")` etc. to the orchestrator stages per spec Â§6.5
...
### Task 7.4 â€” RAGAS test module `tests/eval/ragas/test_retrieval_quality.py`

- **Red (for the test file itself)**: write the test as spec Â§6.3 shows. When run against the synthetic corpus, it must fail with a missing-fixtures error first, then pass once Phase 0 fixtures are loaded.
- **Green:** implement the test. Use the judge path verified in Task 0.5.
- **Commit:** `test(rag): RAGAS retrieval quality eval`

### Task 7.5 â€” RAGAS test module `tests/eval/ragas/test_answer_quality.py`

- Similar structure, measures answer-quality metrics (Faithfulness, AnswerRelevancy, AnswerCorrectness)
- Threshold assertions per spec Â§6.3
- `test(rag): RAGAS answer quality eval`

### Task 7.6 â€” GitHub Actions workflow `.github/workflows/rag-eval.yml`

- Copy spec Â§6.3 CI job shape
- Use `paths:` filter on `website/features/rag_pipeline/**` and `supabase/website/rag_chatbot/**`
...
**Rollback:** disable `@trace_stage` via a setting flag (e.g. `rag_tracing_enabled=False` â†’ decorator becomes a no-op). Sidecar stays up but unused. CI workflow: remove the `paths:` filter temporarily to stop triggering.

---

## Phase 8 â€” Hardening

**Goal:** Monitor real usage for 1â€“2 weeks, fix top issues, raise eval thresholds as metrics stabilize, and announce publicly.

**Dependencies:** Phases 1â€“7 complete.

This phase has no fixed task list â€” it's a monitoring and triage phase. Expected activities:
...
- Define alert thresholds per spec Â§6.6:
...
- Once 2 weeks of stable metrics: update the README (minimal addition â€” no env var lists per user memory), announce via whatever channel makes sense
...
## Appendix A â€” Task-to-spec-section cross-reference

| Task | Spec Â§ | Blueprint gap fix |
|---|---|---|
| 1.1 HNSW migration | Â§2.1 | â€” |
| 1.2 Chunks table | Â§2.2 | â€” |
| 1.3 Sandboxes | Â§2.3 | â€” |
| 1.4 Chat sessions | Â§2.4 | â€” |
| 1.5 RPCs | Â§2.5 | #2 (graph depth in rag_hybrid_search) |
| 2.6 Chunker | Â§3.2 | #3 (atomic entity enrichment) |
| 2.7 Embedder | Â§3.2 (patched) | â€” |
| 2.8 Upsert | Â§3.2 (patched) | #4 (content-hash skip) |
| 2.9 Telegram wiring | Â§3.2 end-of-section | â€” |
| 3.3 QueryTransformer | Â§3.3 | #1 (STEP_BACK query class) |
| 3.4 LRUCache | Â§3.4 | #5 (query + retrieval cache) |
| 3.7 Graph depth config | Â§3.4 | #2 (graph depth) |
| 3.10 TEIReranker | Â§3.6 | â€” |
| 3.11 ContextAssembler | Â§4.1 | â€” |
| 3.13 Gemini streaming | Â§3.7.3 | â€” |
| 3.18 Answer Critic | Â§4.5 | â€” |

All 5 blueprint-gap fixes from spec Â§3 are grounded in specific tasks.

---

## Appendix B â€” Commit-prefix cheat sheet

| Prefix | Use for |
|---|---|
| `feat(rag)` | new Python module in `website/features/rag_pipeline/**` |
...
## Appendix C â€” Sanity-check greps before declaring a phase complete
...
grep -rE "^(from|import) google" website/features/rag_pipeline/ | grep -v adapters/

# 2. No v2 Langfuse imports
grep -r "langfuse.decorators" website/features/rag_pipeline/ website/features/rag_chatbot/
...
grep -rn "key_pool\._" website/features/rag_pipeline/ | grep -v adapters/
...
## Appendix D â€” Gap-fix addendum (16 items from spec-vs-plan audit)
...
**Task 2.0 â€” Update `ops/requirements.txt` with new runtime deps**
...
**Task 2.0b â€” Create `website/features/rag_pipeline/__init__.py` package marker**

Create `website/features/rag_pipeline/__init__.py` (empty, or with a version string). Without this, every `from website.features.rag_pipeline import ...` fails. Also create `website/features/rag_pipeline/ingest/__init__.py` and `website/features/rag_pipeline/adapters/__init__.py`. Run before Task 2.2.

**Commit:** `chore(rag): init package markers`

---

**Task 2.11 â€” Create `ops/scripts/backfill_chunks.py` skeleton**

Spec Â§9 (parked features) says to create a skeleton backfill script. Write the argparse shell + a `TODO: iterate Obsidian markdown files, extract raw text, chunk, embed, upsert` body. No implementation. This documents the future path.

```python
# ops/scripts/backfill_chunks.py
"""
Backfill kg_node_chunks for existing Zettels from Obsidian markdown files.
NOT part of the v1 pipeline â€” run manually when summary-only fallback
causes retrieval misses on old content. See spec Â§9.
"""
import argparse
...
**Task 3.26 â€” Create `website/features/rag_pipeline/backends/` package with `websearch.py` stub**

Spec Â§8.1 and Â§9 explicitly call for this parked stub:

```python
# website/features/rag_pipeline/backends/__init__.py
# (empty)

# website/features/rag_pipeline/backends/websearch.py
"""
PARKED: Future web-search popup feature.
Spec Â§9: "future ad-hoc info-needs feature, reserved namespace."
"""
class WebSearchBackend:
    async def search(self, query: str) -> str:
        raise NotImplementedError(
            "WebSearchBackend is parked for v2. See spec Â§9."
        )
```
...
**Task 3.27 â€” Update deploy script with reranker health-check preflight**

Spec Â§7.2 says: before Caddy upstream swap, run `docker compose run --rm reranker wget -qO- http://localhost:8080/health`. Add this line to `ops/deploy/deploy.sh` **after** the new containers are up but **before** the Caddy swap.
...
**Task 5.0 â€” Create `website/features/rag_chatbot/__init__.py`**
...
**Commit:** fold into Task 5.5 (minimal chat page) â€” no separate commit needed.

---

### Phase 6 additions

**Task 6.24 â€” Create `content/example_queries.json` for empty-state UX**

Spec Â§5.5 lists this file. Write 10â€“15 sample queries the UI shows when the user opens `/chat` with no history (spec Â§5.8 "first-time empty state"). Examples:
...
**Phase 6 verification gate â€” add 2 edge-case notes:**

- **Edge case #36 (multiple browser tabs):** manually open `/chat` in 2 tabs, send a message in tab 1, switch to tab 2, verify it shows the new turn after clicking into the session list or refreshing. No dedicated test needed â€” note this as a manual check in the Phase 6 gate.
...
**Task 7.0 â€” Update `ops/requirements.txt` + `ops/requirements-dev.txt` for eval deps**
...
**Task 7.3b â€” Create `tests/eval/ragas/conftest.py`**
...
**Task 7.2b â€” Create `website/features/rag_pipeline/observability/metrics.py`**

Spec Â§8.1 lists this file. Create it with per-stage latency tracking helpers used by `@trace_stage`:

```python
# website/features/rag_pipeline/observability/metrics.py
import time
...
**Task 7.2c â€” Configure Langfuse client sanitization**

Spec Â§6.7 says: "Langfuse `sanitize` function strips keys matching `{api_key, token, password, secret}`." Implement this as a custom callback or a pre-send filter on the Langfuse client:

```python
# In website/features/rag_pipeline/observability/tracer.py, after get_client():
import re
...
**Task 8.5 â€” Implement L3 5% production sampling**

Spec Â§6.8 says: "5% sampling of chat turns â†’ gemini-2.5-flash-lite runs a lightweight faithfulness check on every 20th turn." Implement as a post-generation decorator or a counter-based check inside the orchestrator's `_run_pipeline`:
...
**Task 8.6 â€” Create quarterly blueprint re-read calendar entry**

Spec Â§6.8 and Â§13 #10 describe this as an operational discipline item. Create a note:
...
**Task 8.7 â€” Document pre-computed Leiden community summaries migration path**

Spec Â§9 parked feature says "migration placeholder documented." Add a short markdown section to the spec or a separate doc describing:
...
**Task 3.25 addendum â€” Edge case #28 (answer exceeds max_output_tokens)**
...
| # | Phase | Task | Spec Â§ | What's added |
|---|---|---|---|---|
| 1 | 2 | 2.0 | Â§8.2 | `ops/requirements.txt` chonkie + networkx |
| 2 | 2 | 2.0b | Â§8.1 | `__init__.py` package markers for `rag/`, `ingest/`, `adapters/` |
| 3 | 2 | 2.11 | Â§9 | `ops/scripts/backfill_chunks.py` skeleton |
| 4 | 3 | 3.26 | Â§8.1, Â§9 | `backends/websearch.py` stub + `__init__.py` |
| 5 | 3 | 3.27 | Â§7.2 | Reranker health-check in deploy script |
| 6 | 3 | 3.25+ | Â§12 #28 | Edge case: answer exceeds max_output_tokens |
| 7 | 5 | 5.0 | Â§5.5 | `rag_chatbot/__init__.py` package marker |
| 8 | 6 | 6.24 | Â§5.5 | `content/example_queries.json` |
| 9 | 6 | gate | Â§12 #36, #44 | Manual edge-case checks added to verification gate |
| 10 | 7 | 7.0 | Â§8.2, Â§8.3 | `ops/requirements.txt` langfuse + `ops/requirements-dev.txt` ragas/respx |
| 11 | 7 | 7.3b | Â§6.2 | `tests/eval/ragas/conftest.py` |
| 12 | 7 | 7.2b | Â§8.1 | `observability/metrics.py` latency helper |
| 13 | 7 | 7.2c | Â§6.7 | Langfuse payload sanitization |
| 14 | 8 | 8.5 | Â§6.8 | L3 5% faithfulness sampling |
| 15 | 8 | 8.6 | Â§6.8, Â§13 #10 | Quarterly blueprint re-read schedule |
| 16 | 8 | 8.7 | Â§9 | Leiden community summaries migration recipe |
