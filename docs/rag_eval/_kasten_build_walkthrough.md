...
| 6 | Bypass via direct `POST /api/zettels/add` for zettel #2 (`https://github.com/zk-org/zk`) | 34.2 s | ✅ `gh-zk-org-zk` "zk-org/zk" (github) | — |
...
| **UX-4** | High | YouTube ingestion returns flat 422 when transcript access is restricted, **even though** the GeminiKeyPool documentation claims "it can bypass transcript extraction and send the video URL directly to Gemini's video understanding API" (per `CLAUDE.md` § API Key Pool & Model Fallback). The fallback path is not wired for the website's `/api/zettels/add` route. | Steps 7-8 — two real PKM-topic videos failed with the same generic error; only worked for a third video that happened to have transcripts | iter-02: wire video-understanding fallback into `/api/zettels/add` (not just the bot path); update error copy to be specific ("Tried transcript + video API; both denied") |
